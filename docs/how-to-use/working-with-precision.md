...
Exact numbers can be entered for keyframes and handles.`Updated in v1.5`
...
- **Double-click on any value in the under-graph settings** to change that value.`New in v1.5`
- **Double-click on graph limit values** to change them. `New in v1.5`
- In all these cases, **type in new values** to change them, then approve the change by pressing `RETURN` or `ENTER`, or cancel the change with `ESCAPE`.
...
- When numeric entry is active or when the under-graph values are being edited, use **left and right arrows** to move to the previous or next keyframe or handle. `New in v1.5`
